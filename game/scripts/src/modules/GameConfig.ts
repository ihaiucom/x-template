...
export class GameConfig {
    constructor() {
        print('[zf]GameConfig constructor');
        ...
        game.SetCustomHeroMaxLevel(32); // 设置自定义英雄最大等级
        game.SetCustomXPRequiredToReachNextLevel({
            // 设置自定义英雄每个等级所需经验，这里的经验是升级到这一级所需要的*总经验）
            1: 0,
        });
        game.SetDaynightCycleDisabled(false); // 是否禁用白天黑夜循环
        game.SetDeathOverlayDisabled(false); // 是否禁用死亡遮罩（灰色的遮罩）
        ...
        let gameModel = new GameMode();
        print('[zf]gameModel='+ gameModel);
        GameMode.Activate();
    }
}
